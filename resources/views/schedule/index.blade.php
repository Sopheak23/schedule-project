    ...
    <table class="table table-bordered" style="font-size:10px;text-align:center;vertical-align: middle;">
        <thead>
            ...
            {{-- span of room number column --}}
                ...
                {{-- show days --}}
                @foreach ($days as $day)
                    <th class="mainDay_{{$day->day}}" colspan="3">{{$day->day}}</th>
                @endforeach
            </tr>
            <tr>
                                    ...
                                    @for ($i = 0; $i < 1; $i++)
                                        <td  style="padding:0; white-space: normal;">
                                            ...
                                            @foreach ($assigned_rooms as $assigned_room)
                                                @if ($assigned_room->assigned_start_time->time === $test_time->morning && $assigned_room->day->day == $day->day && $assigned_room->room->room_name == $room->room_name)
                                                    <button type="button" data-time="{{$test_time->morning}}" data-day="{{$day->day}}" data-day_id="{{$day->id}}" data-room_id="{{$room->id}}" data-room="{{$room->room_name}}"  class="btn btn-light  btn-xs col-lg-12" style="font-size: 12px;border-radius:0px; width:70px; height:30px; white-space: normal; background-color:#dc3545; border-color:#dc3545; color:white;" data-toggle="modal" data-target="#myModal" id="myButton" disabled="true" >
                                            ...
                                            @endforeach
                                            @if ($assigned === false)
                                                <button type="button" data-time="{{$test_time->morning}}" data-day="{{$day->day}}" data-day_id="{{$day->id}}" data-room_id="{{$room->id}}" data-room="{{$room->room_name}}"  class="btn btn-light" style="font-size: 12px;border-radius:0px;width:70px; height:30px;" data-toggle="modal" data-target="#myModal" id="myButton">
                                        ...
                                        <td  style="padding:0;">
                                            <?php $assigned = false; ?>
                                            @foreach ($assigned_rooms as $assigned_room)
                                                @if ($assigned_room->assigned_start_time->time === $test_time->afternoon && $assigned_room->day->day == $day->day && $assigned_room->room->room_name == $room->room_name)
                                                    <button type="button" data-time="{{$test_time->morning}}" data-day="{{$day->day}}" data-day_id="{{$day->id}}" data-room_id="{{$room->id}}" data-room="{{$room->room_name}}"  class="btn btn-light  btn-xs col-lg-12" style="font-size: 12px;border-radius:0px;width:70px; height:30px; white-space: normal; background-color:#dc3545; border-color:#dc3545; color:white;" data-toggle="modal" data-target="#myModal" id="myButton" disabled="true" >
                                                        {{-- <span style="font-size: 12px;padding-bottom:5px;width: 70px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis"></span> --}}
                                                        {{$assigned_room->class->subject->subject_name}}
                                                        <?php $assigned = true; ?>
                                            ...
                                            @endforeach
                                            @if ($assigned === false)
                                                <button type="button" data-time="{{$test_time->afternoon}}" data-day="{{$day->day}}" data-day_id="{{$day->id}}" data-room_id="{{$room->id}}" data-room="{{$room->room_name}}"  class="btn btn-light" style="font-size: 12px;border-radius:0px;width:70px; height:30px;" data-toggle="modal" data-target="#myModal" id="myButton">
                                                    {{-- <span style="font-size: 12px;padding-bottom:5px;width: 70px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis"></span> --}}
                                                    {{$test_time->afternoon}}
                                                </button>
                                            @endif
                                        ...
                                        <td  style="padding:0;">
                                            ...
                                            @foreach ($assigned_rooms as $assigned_room)
                                                <?php $time = null; ?>
                                                @if ($assigned_room->assigned_start_time->time === $test_time->evening && $assigned_room->day->day == $day->day && $assigned_room->room->room_name == $room->room_name)
                                                    <button type="button" data-time="{{$test_time->morning}}" data-day="{{$day->day}}" data-day_id="{{$day->id}}" data-room_id="{{$room->id}}" data-room="{{$room->room_name}}"  class="btn btn-light  btn-xs col-lg-12" style="font-size: 12px;border-radius:0px;width:70px; height:30px; white-space: normal; background-color:#dc3545; border-color:#dc3545; color:white;" data-toggle="modal" data-target="#myModal" id="myButton" disabled="true" >
                                                        {{-- <span style="font-size: 12px;padding-bottom:5px;width: 70px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis"></span> --}}
                                                        {{$assigned_room->class->subject->subject_name}}
                                                        <?php $assigned = true; ?>
                                            ...
                                            @endforeach
                                            @if ($assigned === false)
                                                <button type="button" data-time="{{$test_time->evening}}" data-day="{{$day->day}}" data-day_id="{{$day->id}}" data-room_id="{{$room->id}}" data-room="{{$room->room_name}}"  class="btn btn-light" style="font-size: 12px;border-radius:0px;width:70px; height:30px;" data-toggle="modal" data-target="#myModal" id="myButton">
                                                    {{-- <span style="font-size: 12px;padding-bottom:5px;width: 70px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis"></span> --}}
                                                    {{$test_time->evening}}
                                                </button>
                                            @endif
                    ...
                    <div class="form-group row" style="margin: 40 0 40 50px; width: 600px; height: 50px;margin-bottom: 40px; font-weight:bold;">
                        ...
                        <div class="col-sm-10">
                            <select type="text" class="form-control" id="class_id" name="class_id" placeholder="Class Name">
                                <option id="class_init_opt" value="">Subject Name</option>
